...
from app.bot.messages import asset_heading
...
async def create_alert_from_candidate(
    ...
    *,
    ...
    try:
        alert = await create_alert_from_command(
            session,
            user_id=user.id,
            parsed=parsed,
    ...
    await _send_result(
        ...
        t(
            ...
            market=with_icon(
                asset_icon(asset.type, asset.chain, (asset.extra or {}).get("exchange")), escape(asset_kind_label(asset))
            ),
...
def candidate_heading(candidate: AssetCandidate) -> str:
    parts = [candidate.name or candidate.symbol]
    if candidate.chain:
        parts.append(candidate.chain)
    if price := candidate.metadata.get("price_usd"):
        parts.append(f"${price}")
    icon = asset_icon(candidate.type.value, candidate.chain, candidate.metadata.get("exchange"))
    return asset_heading(" - ".join(parts), icon)


def supports_market_cap(candidate: AssetCandidate) -> bool:
    if candidate.provider == "dexscreener":
        return True
    return candidate.provider == "hyperliquid" and candidate.metadata.get("market") == "spot"


def _format_condition(parsed: ParsedAlertCommand, currency: str) -> str:
    if parsed.alert_type == AlertType.PERCENT_CHANGE:
        return t(f"condition-percent-{parsed.direction.value}", threshold=format_percent(parsed.threshold_value))
    key = {
